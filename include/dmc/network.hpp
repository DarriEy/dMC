...
struct ChannelGeometry {
    ...
    Real width(const Real& Q) const {
        return width_coef * safe_pow(safe_max(Q, Real(0.01)), to_double(width_exp));
    }
    
    Real depth(const Real& Q) const {
        return depth_coef * safe_pow(safe_max(Q, Real(0.01)), to_double(depth_exp));
    }
    ...
    Real wetted_perimeter(const Real& Q) const {
        Real w = width(Q);
        Real d = depth(Q);
        return w + 2.0 * d;  // Rectangular approximation
    }
