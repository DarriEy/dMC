...
class WorkingDirectory:
    ...
    def __init__(self, original_config: str):
    ...
    def setup(self) -> 'WorkingDirectory':
        ...
        self.work_topology = self.work_dir / 'topology.nc'
        shutil.copy(topology_path, self.work_topology)
        ...
        with open(self.work_config, 'w') as f:
            for line in config_lines:
                if '<fname_ntopo>' in line:
                    f.write(f"<fname_ntopo>  {self.work_topology}\n")
                elif '<network_file>' in line:
                    f.write(f"<network_file>  {self.work_topology}\n")
                elif '<fname_output>' in line:
...
def update_manning_n_in_topology(topology_path: str, reach_ids: np.ndarray, 
    ...
    try:
        with nc.Dataset(str(topology_path), 'r+') as ds:
            ...
            if 'mann_n' not in ds.variables:
                ...
                ds.variables['mann_n'][:] = np.full(len(seg_ids), 0.03)
            
            # Create mapping from reach_id to index
            id_to_idx = {int(sid): i for i, sid in enumerate(seg_ids)}
            
            # Update values
            n_values = ds.variables['mann_n'][:].copy()
            for rid, n_val in zip(reach_ids, manning_n):
                if int(rid) in id_to_idx:
                    n_values[id_to_idx[int(rid)]] = float(n_val)
            
            ds.variables['mann_n'][:] = n_values
        return True
    except Exception as e:
        print(f"Warning: Failed to update topology.nc: {e}")
        return False


def run_dmc_route(exe: str, config: str, verbose: bool = False) -> bool:
    """Run dMC-Route and return success status."""
    result = subprocess.run([exe, '-c', str(config)], capture_output=True, text=True)
    if verbose:
        print(result.stdout)
...
class DMCRouteFunction(torch.autograd.Function):
    ...
    def forward(ctx, manning_n: torch.Tensor, 
        ...
        # Update Manning's n in working topology
        manning_n_np = manning_n.detach().numpy()
        update_manning_n_in_topology(work_dir.work_topology, reach_ids, manning_n_np)
        
        # Run model with working config
        if not run_dmc_route(exe, work_dir.work_config, verbose=False):
...
def main():
    ...
    work_dir = WorkingDirectory(args.config)
    ...
    try:
        ...
        # Initial run
        print(f"\nRunning dMC-Route...")
        if not run_dmc_route(args.exe, work_dir.work_config):
